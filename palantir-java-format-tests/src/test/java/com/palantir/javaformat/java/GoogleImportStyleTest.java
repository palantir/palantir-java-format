...
public class GoogleImportStyleTest {
            ...
            {
                {
                    ...
                    "import static com.google.assertThat;",
                    "",
    ...
    public void reorder() throws FormatterException {
        try {
            ...
            assertThat(reordered).withFailMessage("Expected exception").doesNotMatch("^!!");
            assertThat(output).withFailMessage(input).isEqualTo(reordered);
        } catch (FormatterException e) {
            if (!reordered.startsWith("!!")) {
                throw e;
            }
            assertThat(reordered).endsWith("\n");
            assertThat(e).hasMessage("error: " + reordered.substring(2, reordered.length() - 1));
        }
    }
}
