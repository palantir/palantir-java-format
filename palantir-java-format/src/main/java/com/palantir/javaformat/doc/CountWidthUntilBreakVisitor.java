...
class CountWidthUntilBreakVisitor implements DocVisitor<Float> {
  ...
  /**
   * @param availableWidth the max width we will accept a suffix can take before ignoring it /
   *     trying to split it anyway.
   */
  ...
  /**
   * Gets the index of the first {@link Level} that is not empty. If other kinds of {@link Doc}s are
   * encountered, returns {@link OptionalInt#empty}.
   */
