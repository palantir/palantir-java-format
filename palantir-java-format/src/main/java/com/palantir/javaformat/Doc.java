...
/**
 * {@link com.palantir.javaformat.java.JavaInputAstVisitor JavaInputAstVisitor} outputs a sequence
 * of {@link Op}s using {@link OpsBuilder}. This linear sequence is then transformed by {@link
 * DocBuilder} into a tree-structured {@code Doc}. The top-level {@code Doc} is a {@link Level},
 * which contains a sequence of {@code Doc}s, including other {@link Level}s. Leaf {@code Doc}s are
 * {@link Token}s, representing language-level tokens; {@link Tok}s, which may also represent
 * non-token {@link Input.Tok}s, including comments and other white-space; {@link Space}s,
...
public abstract class Doc {
  ...
  public static final class State {
    ...
    State withBreak(Break brk) {
      int newColumn = Math.max(indent + brk.getPlusIndent(), 0);
  ...
  static final class Level extends Doc {
    ...
    private void assertStartsWithBreakOrEmpty(Doc doc) {
      Preconditions.checkState(
          new StartsWithBreakVisitor().visit(doc) != Result.NO,
     ...
     *
     * <p>This relies on {@link #splitByBreaks} having been called beforehand so that {@link
     * #splits} and {@link #breaks} are set.
     */
