...
public enum BreakBehaviour {
  ...
  /**
   * Break if by doing so all inner levels then fit on a single line. However, don't break if we can
   * fit in the {@link Doc docs} up to the first break (which might be nested inside the next doc if
   * it's a {@link Level}), in order to prevent exceeding the maxLength accidentally.
   */
