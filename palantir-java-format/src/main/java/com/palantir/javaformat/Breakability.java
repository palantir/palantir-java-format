...
public enum Breakability {
  ...
  NO_PREFERENCE,
  /**
   * Unconditionally allow breaking this level. This should only be used when you know that the
   * first non-Level {@link Doc} inside this level, if you flatten it, is a {@link Break}.
   */
  BREAK_HERE,
  /**
  ...
  CHECK_INNER,
  /**
   * Check whether the <i>first</i> inner level can fit on the same line. This assumes that the next
   * Doc after that starts with a {@link Break} (see {@link StartsWithBreakVisitor}) and makes sense
   * in contexts like {@link JavaInputAstVisitor#visitDotWithPrefix} where we want to treat first
   * doc (the longest prefix) as a single entity to be fit onto the same line.
   */
